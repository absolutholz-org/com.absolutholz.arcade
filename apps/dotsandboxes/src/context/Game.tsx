	...
	switch (action.type) {
		case GameAction.CreateGameboard:
			...
			for (let iRow = 0, lRow = action.rowCount; iRow < lRow; iRow++) {
				for (
					let iCol = 0, lCol = action.columnCount;
					iCol < lCol;
					iCol++
				) {
					squares.push({
						...
						borders: {
							...
							[`${iRow}x${iCol + 1}|${iRow + 1}x${iCol + 1}`]:
								false, // right
							[`${iRow + 1}x${iCol}|${iRow + 1}x${iCol + 1}`]:
								false, // bottom
						},
					});
				}
		...
		case GameAction.ConnectDots:
			...
			for (const square of state.squares) {
				...
				if (square.borders[borderID] !== undefined) {
					...
					isSquareComplete = Object.values(square.borders).every(
						(value) => value === true
					);
					if (isSquareComplete) {
			...
			if (!isSquareComplete) {
				const newCurrentIndex = state.players?.findIndex(
					(player) => player === state.nextPlayer
				);
				currentPlayer = state.players[newCurrentIndex];
				nextPlayer =
					state.players[
						newCurrentIndex + 1 < state.players.length
							? newCurrentIndex + 1
							: 0
					];
...
export function GameProvider({
	...
	const [{ players, currentPlayer, nextPlayer, squares }, dispatch] =
		useReducer(reducer, { players: [], squares: [] });

	return (
		<GameContext.Provider
			value={{ players, currentPlayer, nextPlayer, squares, dispatch }}>
			{children}
		</GameContext.Provider>
	);
}
