...
export function SignInForm({ formId }: ISignInForm): JSX.Element {
	const {
	...
	const handleSignIn = (event: FormEvent) => {
		...
		const elEmail = elForm.querySelector(
			`#${formId}_email`
		) as HTMLInputElement;
		const elPassword = elForm.querySelector(
			`#${formId}_password`
		) as HTMLInputElement;
		console.log(
			{ event, elForm, elEmail, elPassword },
			elEmail.value,
			elPassword.value
		);
	...
	return (
		<form id={formId} onSubmit={handleSignIn}>
			{errorSignInWithEmailAndPassword && (
				<ErrorMessage>
					Sorry, we couldn't sign you in. Please check your email and
